...
### Added
- **Held-out Ripser preset**: Added the narrowly scoped `RIPSER_TUNED` preset
  after a disjoint crossed Atlas/Ripser search and six-dataset, 20-seed H100
  confirmation. It changes only `spread` from 1.0 to 0.8, improved 11/12
  held-out cells against default under each evaluator, and retains documented
  cells where UMAP or t-SNE remains better. No separate `ATLAS_TUNED` preset is
  exposed because the Atlas-selected search candidate did not transfer.
...
- **Unsupported topology preset**: Withdrew the public `TOPOLOGY_TUNED` export
  ...
  did not support its general name; the new `RIPSER_TUNED` preset is a distinct,
  held-out replacement rather than a rename of those parameters.
