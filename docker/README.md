...
```bash
docker build -t dire-rapids:0.3.1 -f docker/Dockerfile .
```
...
```bash
docker buildx build --platform linux/amd64 -t dire-rapids:0.3.1 \
...
```bash
docker run --gpus=all -it --rm -v $PWD:/workspace dire-rapids:0.3.1 bash
```

Run the test suite in a fresh container:

```bash
docker run --gpus=all --rm dire-rapids:0.3.1 \
...
```bash
docker run --gpus=all --rm -v $PWD:/workspace -w /workspace dire-rapids:0.3.1 \
